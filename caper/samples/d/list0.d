...
class Scanner {
    ...
    list0_parser.Token get(out int v)
    {
        int c;
        do {
            c = fgetc(file_);
            if(c == '\n') return list0_parser.Token.token_eof;
        } while (std.ascii.isWhite(c));

        // 記号類
        switch( c ) {
        case '(': return list0_parser.Token.token_LParen;
        case ')': return list0_parser.Token.token_RParen;
        case ',': return list0_parser.Token.token_Comma;
        case EOF: return list0_parser.Token.token_eof;
        default:
            // 整数
            if (std.ascii.isDigit(c)) {
                ...
                ungetc(c, file_);
                v = n;
                return list0_parser.Token.token_Number;
            }
        }
...
int main(char [][] args)
{
    ...
    alias list0_parser.Parser!(int, SemanticAction) Parser;
    auto parser = new Parser(new SemanticAction);

    printf(">");

    list0_parser.Token token;
    int v;
    do{
        token = s.get(v);
