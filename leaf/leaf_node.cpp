    ...
    const symmap_t& freevars )
{
    ...
    llvm::Value* v = check_value_1( value ); // 暫定
...
void IfThenElse::encode( EncodeContext& cc, bool drop_value, Value& value )
{
    ...
    llvm::Value* tvalue = check_value_1( value ); // 暫定
    value.clear();
    ...
    llvm::Value* fvalue = check_value_1( value ); // 暫定
    value.clear();
    ...
    value.assign( loaded_result, Type::getIntType() ); // TODO: 決めうち
}
...
void FunCall::encode( EncodeContext& cc, bool, Value& value )
{
    ...
    if( Type::isFunction( r.t ) ) {
        ...
        for( size_t i = 0 ; i < aargs->v.size() ; i++ ) {
            aargs->v[i]->encode( cc, false, value ); // 暫定
            llvm::Value* vv = check_value_1( value );
            value.clear();
...
*/
        ...
        for( size_t i = 0 ; i < aargs->v.size() ; i++ ) {
            ...
            llvm::Value* vv = check_value_1( value ); // 暫定
            value.clear();
