...
void IfThenElse::encode( EncodeContext& cc, bool drop_value, values_t& values )
{
    ...
    llvm::Value* tvalue = check_values_1( values ); // 暫定
    values.clear();
    if( ifresult ) {
        new llvm::StoreInst( 
            tvalue,
    ...
    llvm::Value* fvalue = check_values_1( values ); // 暫定
    values.clear();
    if( ifresult ) {
        new llvm::StoreInst(
            fvalue,
...
void FunCall::encode( EncodeContext& cc, bool, values_t& values )
{
    ...
    if( Type::isFunction( v.t ) ) {
        ...
        for( size_t i = 0 ; i < aargs->v.size() ; i++ ) {
            aargs->v[i]->encode( cc, false, values ); // 暫定
            llvm::Value* vv = check_values_1( values );
            values.clear();
...
*/
        ...
        for( size_t i = 0 ; i < aargs->v.size() ; i++ ) {
            ...
            llvm::Value* vv = check_values_1( values ); // 暫定
            values.clear();
